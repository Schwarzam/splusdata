...
FIELD_FRAME_ARGS = {
    ...
    "weight": [
        ...
        dict(
            action="store_true",
            default=False,
            help="Download the weight map associated to the stamp.",
        ),
    ],
    ...
    "field": [
        "pos",
        dict(metavar="FIELD", help='Field identifier, e.g., "SPLUS-n01s10"'),
    ],
    "band": [
        "pos",
        dict(metavar="BAND", help='S-PLUS band, e.g., "R", "I", "F660", "U".'),
    ],
}
...
def field_frame():
    ...
    parser = create_parser(
        args_dict=FIELD_FRAME_ARGS, program_description=FIELD_FRAME_PROG_DESC
    )
...
STAMP_ARGS = {
    ...
    "size_unit": [
        ...
        dict(
            metavar="SIZE_UNIT",
            choices=["arcsec", "arcmin", "pixels"],
            default="pix",
            help="Unit of the size parameter (arcsec, arcmin, pixels).",
        ),
    ],
    "data_release": ["D", dict(default="DR4", help="S-PLUS data release version.")],
    "outfile": [
        "o",
        dict(default="stamp.fits.fz", help="Output filename for the stamp FITS image."),
    ],
    # positional arguments
    ...
    "size": [
        ...
        dict(
            metavar="SIZE",
            help="Size of the stamp in size_unit (defaults to pixels) (see --size_unit).",
        ),
    ],
    "band": [
        "pos",
...
def stamp():
    ...
    conn = Core(args.username, args.password)
    print_level(
        f"Downloading stamp at RA: {args.ra}, DEC: {args.dec} - band {args.band}",
        1,
        args.verbose,
    )
    conn.stamp(
...
LUPTON_RGB_ARGS = {
    ...
    "rgb": [
        ...
        dict(
            ...
            nargs=3,
            default="I,R,G",
            help="Space-separated bands for RGB channels (default: I R G).",
        ),
    ],
    "field_name": [
        "F",
    ...
    "outfile": [
        "o",
        dict(
            default="lupton_RGB.png", help="Output filename for the Lupton RGB image."
        ),
    ],
    "Q": ["Q", dict(type=float, default=8, help="Lupton Q parameter (default: 8).")],
    "stretch": [
        "s",
        dict(type=float, default=3, help="Lupton stretch parameter (default: 3)."),
    ],
    # positional arguments
    "ra": ["pos", dict(metavar="RA", help="Object's right ascension")],
    "dec": ["pos", dict(metavar="DEC", help="Object's declination")],
...
def lupton_rgb():
    ...
    """
    parser = create_parser(
        args_dict=LUPTON_RGB_ARGS, program_description=LUPTON_RGB_PROG_DESC
    )
    ...
    print_level(
        f"Downloading Lupton RGB image at RA: {args.ra}, DEC: {args.dec}",
        1,
        args.verbose,
    )
    conn.lupton_rgb(
...
TRILOGY_IMAGE_ARGS = {
    ...
    "red": [
        ...
        dict(
            ...
            default="I",
            nargs="+",
            help="Band(s) for the red channel (default: I R G).",
        ),
    ],
    "green": [
        "G",
        dict(
            ...
            default="R",
            nargs="+",
            help="Band(s) for the green channel (default: R G B).",
        ),
    ],
    "blue": [
        "B",
        dict(
            ...
            default="G",
            nargs="+",
            help="Band(s) for the blue channel (default: G B I).",
        ),
    ],
    "field_name": [
        "F",
    ...
    "outfile": [
        "o",
        dict(
            default="trilogy_image.png",
            help="Output filename for the Lupton RGB image.",
        ),
    ],
    "noiselum": [
        "n",
        dict(type=float, default=0.15, help="Controls noise luminance suppression"),
    ],
    "satpercent": [
        "s",
        dict(
            type=float,
            default=0.15,
            help="Percentile value for saturation clipping (default: 0.15).",
        ),
    ],
    "colorsatfac": [
        "c",
        dict(type=float, default=2, help="Factor for color saturation (default: 2)."),
    ],
    # positional arguments
    "ra": ["pos", dict(metavar="RA", help="Object's right ascension")],
    "dec": ["pos", dict(metavar="DEC", help="Object's declination")],
...
def trilogy_image():
    ...
    """
    parser = create_parser(
        args_dict=TRILOGY_IMAGE_ARGS, program_description=TRILOGY_IMAGE_PROG_DESC
    )
    args = parser.parse_args(args=sys.argv[1:])
    ...
    print_level(
        f"Downloading Trilogy RGB image at RA: {args.ra}, DEC: {args.dec}",
        1,
        args.verbose,
    )
    conn.trilogy_image(
...
def calibrated_stamp():
    ...
    """
    parser = create_parser(
        args_dict=CALIBRATED_STAMP_ARGS, program_description=CALIBRATED_STAMP_PROG_DESC
    )
    args = parser.parse_args(args=sys.argv[1:])

    conn = Core(args.username, args.password)
    print_level(
        f"Downloading calibrated stamp at RA: {args.ra}, DEC: {args.dec} - band {args.band}",
        1,
        args.verbose,
    )
    conn.calibrated_stamp(
        args.ra,
        args.dec,
        args.size,
        args.band,
