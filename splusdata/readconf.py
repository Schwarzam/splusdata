...
def create_stamp_name(info, output_folder):
    filename = os.path.join(
        output_folder, f"{info['ra']}_{info['dec']}_{info['size']}_{info['band']}.fits"
    )
    return filename
...
def create_image_name(info, output_folder):
    filename = os.path.join(
        output_folder, f"{info['ra']}_{info['dec']}_{info['size']}.png"
    )
    return filename
...
def handle_operation_type(operation_data, conn):
    ...
    if len(checkcoords_list) > 0:
        ...
        checkcoords_df.to_csv(
            os.path.join(operation_data["output_folder"], "checkcoords.csv"),
            index=False,
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="splusdata - Download SPLUS catalogs, FITS and more"
    )
    parser.add_argument(
        "config_file", metavar="config_file", type=str, help="Configuration file"
    )
