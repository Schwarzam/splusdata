...
def get_fits(name, ra, dec, cut_size, band, filename=None, overwrite=False):
    ...
    response = urlopen(f'http://splus.cloud/api/{name}/{ra}/{dec}/{cut_size}/{band}/0')
    name_in_query = response.read().decode('utf-8')

    response = urlopen(f'http://splus.cloud/api/{name_in_query}/{ra}/{dec}/{cut_size}/{band}/1')
    ...
    if int(queue) == 0:
        response = urlopen(f'http://splus.cloud/api/{name_in_query}/{ra}/{dec}/{cut_size}/{band}/2')
        queue = response.read().decode('utf-8')
