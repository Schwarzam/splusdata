...
class SCubes:
    def __init__(
        self, ra, dec, field, size=None, username=None, password=None, verbose=1
    ):
    ...
    def _getval(self, obj, key, ext):
        return (
            _getval_array_mem(obj, key, ext)
            if self.mem
            else _getval_array(obj, key, ext)
        )
    ...
    def _getheader(self, obj, ext):
        return (
            _getheader_array_mem(obj, ext) if self.mem else _getheader_array(obj, ext)
        )
    ...
    def _download_calibrated_stamps(self, objname, force=False):
        ...
        for b in tqdm(
            BANDS,
            desc=f"{objname} @ {self.field} - Downloading",
            leave=True,
            position=0,
        ):
            ...
            if self.mem:
                ...
                filename = (
                    f"{objname}_{self.field}_{b}_{self.size}x{self.size}_swp.fits.fz"
                )
            ...
            if self.mem:
                ...
                kw_args["outfile"] = join(
                    self.outpath, filename.replace("swp", "swpweight")
                )
                _ = self.conn.calibrated_stamp(**kw_args)
    ...
    def _photospectra(self, flam_scale=None, ext=1):
        ...
        flam__byx = scale * (fnu__byx * _c / self.wl__b[:, None, None] ** 2).to(
            self.flam_unit
        )
        ...
        eflam__byx = scale * (efnu__byx * _c / self.wl__b[:, None, None] ** 2).to(
            self.flam_unit
        )
    ...
    def _weights_mask_hdu(self):
        ...
        wmask_hdu.header["EXTNAME"] = (
            "WEIMASK",
            "Sum of negative weight pixels (from 1 to 12)",
        )
        return wmask_hdu
    ...
    def _create_cube_hdulist(self, objname, ext=1, overwrite=False):
        ...
        hdr = (
            self.images[0][ext].header
            if self.mem
            else fits.getheader(self.images[0], ext=ext)
        )
