...
def make_RGB_tom(
    ...
    return make_lupton_rgb(
        fR * R, fG * G, fB * B, Q=Q, minimum=minimum, stretch=stretch
    )
...
class read_scube:
    ...
    def _mag_values(self):
        ...
        self.emag_arcsec2__byx = (
            (2.5 * np.log10(np.exp(1))) * self.eflux__byx / self.flux__byx
        )
    ...
    def set_ellipticity(self, pa, ba):
        self.pa = pa
        self.ba = ba
        self.pixel_distance__yx = get_image_distance(
            self.weimask__yx.shape, x0=self.x0, y0=self.y0, pa=self.pa, ba=self.ba
        )
    ...
    def weimask__byx(self):
        return np.broadcast_to(
            self.weimask__yx, (len(self.filters), self.size, self.size)
        )
