...
SCUBES_ARGS = {
    ...
    "force_mem": [
        ...
        dict(
            action="store_true",
            default=False,
            help="Force memory mapping of downloaded input files.",
        ),
    ],
...
def scubes():
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="%(prog)s {version}".format(version=__scubes_version__),
    )
...
SCUBESML_ARGS = {
    ...
    "size_multiplicator": [
        ...
        dict(
            default=10,
            type=float,
            help="Factor to multiply the SIZE__pix value of the masterlist to create the galaxy size. If size is a odd number, the program will choose the closest even integer.",
        ),
    ],
    "min_size": [
        "m",
        dict(
            default=200,
            type=int,
            help="Minimal size of the cube in pixels. If size negative, uses the original value of size calculation.",
        ),
    ],
    "workdir": ["w", dict(default=getcwd(), help="Working directory.")],
    "verbose": ["v", dict(action="count", default=0, help="Verbosity level.")],
    ...
    "sname": [
        "O",
        dict(default=None, metavar="OBJECT_SNAME", help="Object's masterlist SNAME"),
    ],
    "force_mem": [
        "F",
        dict(
...
def scubesml():
    ...
    parser = create_parser(
        args_dict=SCUBESML_ARGS, program_description=SCUBESML_PROG_DESC
    )
    # ADD VERSION OPTION
    parser.add_argument(
        "--version",
    ...
    for sname in sname_list:
        ...
        field = mlcut["FIELD"][0]
        size_pix = max(
            round(args.size_multiplicator * float(mlcut["SIZE__pix"][0]) / 2) * 2,
            args.min_size,
        )
        # print(size_pix)
