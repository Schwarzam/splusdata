...
def filter_bw(hdu):
    ...
    for i in range(15, x1 + 30):
        ...
        interpolate_function = interpolate.interp1d(
            interpol_values[0:2, 0], interpol_values[0:2, 1], kind="linear"
        )
    ...
    for i in range(15, x1 + 30):
        ...
        interpolate_function1 = interpolate.interp1d(
            interpol_values1[0:2, 0], interpol_values1[0:2, 1], kind="linear"
        )
        imagemfinalexpandida[y1 + 15 - 1 : y1 + 30, i] = interpolate_function1(
            y1 + 15 - 1 + numpy.arange(16)
        )
    ...
    for j in range(0, y1 + 30):
        ...
        interpolate_function2 = interpolate.interp1d(
            interpol_values2[0:2, 0], interpol_values2[0:2, 1], kind="linear"
        )
    ...
    for j in range(0, y1 + 30):
        ...
        interpolate_function3 = interpolate.interp1d(
            interpol_values3[0:2, 0], interpol_values3[0:2, 1], kind="linear"
        )
        imagemfinalexpandida[j, x1 + 15 - 1 : x1 + 30] = interpolate_function3(
            x1 + 15 - 1 + numpy.arange(16)
        )
    ...
    for i in range(0, 2 * xf):
        for j in range(0, 2 * yf):
            conta1 = abs(i - xm) / cc
            conta2 = abs(j - ym) / dd
            imfilter2[j, i] = (1.0 / (1.0 + conta1**exporder)) * (
                1.0 / (1.0 + conta2**exporder)
            )
