...
class ClassifyObj:
    ...
    def __init__(
        self, data, model="RF16", return_prob=True, match_irsa=False, verbose=True
    ):
        self.data = data
        self.model = model
        ...
        if self.model == "RF16":
            ...
            if self.return_prob:
                ...
                prob_df = pd.DataFrame(
                    model.predict_proba(self.data[self._morph + self._feat])
                )
            ...
            if self.return_prob:
                if self.verbose:
                    print("Calculating probabilities...")
                prob_df = pd.DataFrame(
                    model.predict_proba(data_nowise[self._morph + self._feat])
                )
                prob_wise_df = pd.DataFrame(
                    model_wise.predict_proba(
                        data_wise[self._morph + self._feat + self._feat_wise]
