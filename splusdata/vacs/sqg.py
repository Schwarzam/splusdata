...
class SQGClass:
    ...
    def __init__(self, model="auto", verbose=False):
        ...
        try:
            if self.model == "RF16":
                if self.verbose:
                    print("Loading model...")
                self.model_splus = requests.get(
                    "https://splus.cloud/files/models/iDR3n4_RF_12S4M"
                )
                ...
                if self.verbose:
                    print("Loading model...")
                self.model_wise = requests.get(
                    "https://splus.cloud/files/models/iDR3n4_RF_12S2W4M"
                )
        ...
        if match_irsa == False:
            ...
            try:
                for (
                    ...
                    self._pos
                ):  # check if RA and DEC are in data to perfom the cross-match
                    data[element]
            except ValueError:
                print(
                    "Please ensure that ",
    ...
    def classify(
        ...
        self._error_wise = [
            columns_wise["w1snr"],
            columns_wise["w2snr"],
            columns_wise["w1sigmpro"],
            columns_wise["w2sigmpro"],
        ]
        ...
        if self.model == "RF16":
            y_pred = pd.DataFrame(
                self.model_splus.predict(data[self._morph + self._feat])
            )
            ...
            if return_prob:
                ...
                prob_df = pd.DataFrame(
                    self.model_splus.predict_proba(data[self._morph + self._feat])
                )
            ...
            ypred_wise = pd.DataFrame(
                self.model_wise.predict(
                    data_wise[self._morph + self._feat + self._feat_wise]
                )
            )  # classify sources with WISE
            ...
            y_pred = pd.DataFrame(
                self.model_splus.predict(data_nowise[self._morph + self._feat])
            )
            y_pred = y_pred.astype("float64")
            ...
            if return_prob:
                if verbose:
                    print("Calculating probabilities...")
                prob_df = pd.DataFrame(
                    self.model_splus.predict_proba(
                        data_nowise[self._morph + self._feat]
                    )
                )
                prob_wise_df = pd.DataFrame(
                    self.model_wise.predict_proba(
